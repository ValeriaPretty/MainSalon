...
import com.example.mainsalon.Admin_make.ZapisMake
...
class Okno_podtverdit_make: AppCompatActivity() {
    ...
    fun nice(){
        binding.buttonNooo.setOnClickListener{
            startActivity(Intent(this, MakeUp::class.java))
            finish()
        }
    }
    fun nice1(data: String, time: String, count: String, ad: ADD4, name: String, work: String, price: String){
        binding.buttonYaaas.setOnClickListener{
            val intent = Intent(this, ZapisMake::class.java).apply {
                putExtra("DATA", data)
                putExtra("TIME", time)
