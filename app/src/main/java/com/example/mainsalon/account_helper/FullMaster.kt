...
class FullMaster : AppCompatActivity() {
    ...
    private fun getIntentFrromParic(){
        val add = intent.getSerializableExtra("ADD") as ADD
        upDateUI(add)
        nice2(add)
    }
    private fun upDateUI(add: ADD){
        fillTextViews(add)
    }
    fun fillTextViews(add: ADD) = with(binding){
    ...
    private fun navigateToOknoPodtverdit(data: String, time: String, count: String, ad:ADD, name: String, work: String, price: String) {
        val intent = Intent(this, Okno_podtverdit::class.java).apply {
            putExtra("DATA", data)
            putExtra("TIME", time)
