...
class FullMasterMake : AppCompatActivity() {
    ...
    private fun getIntentFrromParic() {
        val add = intent.getSerializableExtra("ADD") as ADD4
        upDateUI(add)
        nice2(add)
    }
    private fun upDateUI(add: ADD4) {
        fillTextViews(add)
    }
    fun fillTextViews(add: ADD4) = with(binding) {
    ...
    private fun nice2(add: ADD4) {
        binding.timeZapisi11.setOnClickListener {
            ...
            if (isUserAuthenticated()) {
                ...
                Toast.makeText(this, this.resources.getString(R.string.auth), Toast.LENGTH_LONG)
                    .show()
            }
        }
    ...
    private fun navigateToOknoPodtverdit(data: String, time: String, count: String, ad: ADD4, name: String, work: String, price: String) {
        val intent = Intent(this, Okno_podtverdit_make::class.java).apply {
            putExtra("DATA", data)
            putExtra("TIME", time)
