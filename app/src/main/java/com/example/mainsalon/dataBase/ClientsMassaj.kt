...
class ClientsMassaj(val context: Context, val readDataCallBack: ReadDataCallBackClient) {
    val db = Firebase.database.getReference("clients")
    val databas = Firebase.database.getReference("main_masaj") // указывает на путь
    val fullMaster = FullMasterMAssaj()
    val auth = Firebase.auth
    val user = FirebaseAuth.getInstance().currentUser
    val email = user!!.email
    fun publishClients(ad: ADD2, finishListener:FinishOnWorkListenerClient, add: ADD3) {
    ...
    fun deleteButton(add: ADD3, count: String, finishListener: FinishOnWorkListenerClient) {
    ...
    fun startNewActivity(context: Context) {
        val intent = Intent(context, Massaj::class.java)
        ...
        db.addListenerForSingleValueEvent(object: ValueEventListener { // один раз с этого пути считывает, само по себе не считывает
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (item in snapshot.children) {
                    val client_data = item.children.iterator().next().child("client_data").getValue(
                        ADD2::class.java)
