...
class ClientsMake(val context: Context, val readDataCallBack: ReadDataCallBackClient) {
    val db = Firebase.database.getReference("clients")
    val databas = Firebase.database.getReference("main_make") // указывает на путь
    val fullMaster = FullMasterMake()
    val auth = Firebase.auth
    val user = FirebaseAuth.getInstance().currentUser
    val email = user!!.email
    fun publishClients(ad: ADD2, finishListener:FinishOnWorkListenerClient, add: ADD4) {
    ...
    fun deleteButton(add: ADD4, count: String, finishListener: FinishOnWorkListenerClient) {
    ...
    fun startNewActivity(context: Context) {
        val intent = Intent(context, MakeUp::class.java)
